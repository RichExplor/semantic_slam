...
pcl::PointCloud<PointTypeRGB>::Ptr lastCloud(new pcl::PointCloud<PointTypeRGB>());
...
void PoseLoamMsg::callback(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg, const nav_msgs::Odometry::ConstPtr &laserOdometry)
{
    ...
    // 4. 语义分割网络粗略划分静态和动态点云
    pcl::PointCloud<PointTypeRGB>::Ptr fullCloud(new pcl::PointCloud<PointTypeRGB>());
    ...
    for(size_t i=0; i<dnum; i++)
    {
        ...
        point_ori.b =  color_mask[i][0];
        point_ori.g =  color_mask[i][1];
        point_ori.r =  color_mask[i][2];
    ...
    pcl::KdTreeFLANN<PointTypeRGB> kdtreeNear;
    ...
    seeds.clear();
    pcl::PointCloud<PointTypeRGB> dynaCloud_Dynatic;
    ...
    // 8. 真实动态点云区域生长，确保完整性
    pcl::KdTreeFLANN<PointTypeRGB> kdtreeRadius;
    ...
    pcl::PointCloud<PointTypeRGB> dynaCloud_Static;
    pcl::PointCloud<PointTypeRGB> dynaCloud_Dynatic_full;
    for(int i=0; i<dynaNum; ++i)
    {
        point_ori = dynaCloud->points[i];
