...
// template <typename PointT> 
// void voxel_grid_filter(const pcl::PointCloud<PointT>& source_cloud, pcl::PointCloud<PointT>& filtered_cloud, const double& voxel_leaf_size)
// {
...
void PoseLoamMsg::callback(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg, const nav_msgs::Odometry::ConstPtr &laserOdometry)
{
    ...
    // 2. 原始点云的地面分割
    float *fPoint = (float*)calloc(dnum*4, sizeof(float));
    int *pLabel1 = (int*)calloc(dnum, sizeof(int));

    for(int i=0; i<dnum; ++i)
    {
        fPoint[i*4] = points[i][0];
        fPoint[i*4+1] = points[i][1];
        fPoint[i*4+2] = points[i][2];
        fPoint[i*4+3] = points[i][3];
    }
    
    PCSeg pcseg;
    pcseg.GroundSematic(pLabel1, fPoint, dnum);  //pLable = 1表示地面
    
    // 3. 构建非地面点和对应种子
    std::vector<int> seeds;   // 用于区域生长的种子
    std::vector<int> pLabel2; // 非平面点中标签
    int agcnt = 0;
    ...
    for(int i=0; i<dnum; i++)
    {
        point_ori.x = fPoint[i*4];
        point_ori.y = fPoint[i*4+1];
        point_ori.z = fPoint[i*4+2];
        point_ori.intensity = fPoint[i*4+3];

        if(pLabel1[i] == 1)  // 地面点
        {
            GroundCloud->push_back(point_ori);  // 地面点
        }
        else
        {
            noGroundCloud->push_back(point_ori);  // 非地面点

            if(pLabel[i] == 1)
            {
                pLabel2.push_back(1);   // 动态点标签
                seeds.push_back(agcnt); // 记录非地面点中为动态点的id，需要与非地面点对应
            } 
            else
            {
                pLabel2.push_back(0);  // 静态点标签
            }
            agcnt++;
        } 
    }
    ...
    kdtree.setInputCloud(noGroundCloud);
    ...
    while(seeds.size()>0)
    {
        ...
        kdtree.radiusSearch(sid, 0.3, k_inds, k_dis);

        for(int ii=0; ii<k_inds.size();++ii)
        {
            if(pLabel2[k_inds[ii]]==0)  // 将原本错划分为静态点重新聚类为动态点
            {
                pLabel2[k_inds[ii]] = 1;
    ...
    {
        point_ori = noGroundCloud->points[i];
        ...
        {
            // 在半径为 thres 的球形范围内的点被移除掉
            if (point_ori.x * point_ori.x + point_ori.y * point_ori.y + point_ori.z * point_ori.z > 20*20)
                continue;

            if (point_ori.x * point_ori.x + point_ori.y * point_ori.y + point_ori.z * point_ori.z < 5*5)
                continue;
    ...
    // 发布静态点云
    pcl::transformPointCloud(*GroundCloud, *GroundCloud, tform);  // 转换拼接点云 statCloud
    sensor_msgs::PointCloud2 laserCloudOutMsg_stat;

    pcl::toROSMsg(*GroundCloud, laserCloudOutMsg_stat);
