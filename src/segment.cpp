...
int PCSeg::GroundSematic(int *pLabel, float* fPoints, int pointNum)
{
    // 1 先进行地面校正
    float tmpPos[6];
    tmpPos[0]=-0.15;
    ...
    tmpPos[4]=0;
    tmpPos[5]=-2.04;
    GetGndPos(tmpPos,fPoints,pointNum); //tempPos是更新后的地面搜索点 & 平均法向量 ys
    memcpy(this->gndPos,tmpPos,6*sizeof(float));
    
    this->posFlag=1;//(this->posFlag+1)%SELF_CALI_FRAMES;

    // 2 点云矫正
    this->CorrectPoints(fPoints,pointNum,this->gndPos);
    if(this->corPoints!=NULL)
        free(this->corPoints);
    this->corPoints=(float*)calloc(pointNum*4,sizeof(float));
    this->corNum=pointNum;
    memcpy(this->corPoints,fPoints,4*pointNum*sizeof(float));

    // 3 粗略地面分割for地上分割
    int gnum=GndSeg(pLabel,fPoints,pointNum,1.0);  //plabel=1表示地面
...
int PCSeg::DoSeg(int *pLabel1, float* fPoints1, int pointNum)
{
    ...
    {
        if(pLabelGnd[ii]==0) //上一步地面标记为1，非地面标记为0  pLabelGnd = 1 表示地面
        {
    ...
    if (agnum != 0)
    {
        AbvGndSeg(pLabelAg,fPoints3,agnum);  // 非地面点
    }
    else
    {
...
int SegBG(int *pLabel, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree, float fSearchRadius)
{
    ...
    while(seeds.size()>0)
    {
        ...
        for(int ii=0;ii<k_inds.size();ii++)
        {
            ...
            {
                pLabel[k_inds[ii]]=1;
                if(cloud->points[k_inds[ii]].z>0.2)//地面60cm以下不参与背景分割，以防止错误的地面点导致过度分割 0.2
                {
